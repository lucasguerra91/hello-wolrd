...
def traducir(axioma, traducciones, cantidad):
    ...
    :param cantidad: veces que se debe aplicar la traduccion
...
def crear_svg(destino, secuencia, angulo):
    ...
    with open(destino, 'w') as f:
        ...
        tortuguero = p.Pila()
        tortuguero.apilar(t.Tortuga(0, 0, 270, pluma.Pluma(), f))

        dibujar(secuencia, tortuguero, unidad, angulo)

        f.write('</svg>\n')


def dibujar(secuencia, tortuguero, unidad, angulo):
    """
    ...
    for c in secuencia:
        if c == 'F' or c == 'G':
            ...
            tortuguero.ver_tope().circulo(unidad)
